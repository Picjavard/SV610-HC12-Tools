...
public class SV610_HC_12_Tools_by_Arlycad extends PApplet {
...
public void write() {
  ...
  if (!type) {
    ...
    COMPort.write(PApplet.parseInt(channel)); //канал
...
public void readTick() {
  if (COMPort.available() > 0) {
    ...
    for (int i = 0; i < buff.length; i++) {
      if (PApplet.parseChar(buff[i])=='\n')
      {
        println();
        log_println();
        log_print("[String] ");
      } else {
        print(PApplet.parseChar(buff[i]));
        log_print(PApplet.parseChar(buff[i]));
      }
    }
  }
}

public void drawTextLog() {
  readTick();
...
public void log_print(String s) {
  textLog[textLog.length-1]+= s;
}
public void log_print(byte s) {
  textLog[textLog.length-1]+= s;
}
public void log_print(char s) {
  textLog[textLog.length-1]+= s;
}
public void log_print(int s) {
  textLog[textLog.length-1]+= s;
}
public void log_println(String s) {
  textLog[textLog.length-1] += s;
  for (int i = 0; i < textLog.length-1; i++) {
...
//Colors
private int c_very_dark = color(19,66,103); //#134267
private int c_dark = color(28,99,155);//#1C639B
private int c_mid = color(37,131,206);//#2583CE
private int c_light= color(46,159,255);//#2E9FFF

private int c_primary= color(46,159,255);//#2E9FFF
private int c_hover = color(100,184,255);//#64B8FF
...
public void uiSetScale(float scale) {
  ...
  _step_y = PApplet.parseInt((d_height+5) * scale);
}
...
public class DropDown {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "SV610_HC_12_Tools_by_Arlycad" };
    if (passedArgs != null) {
